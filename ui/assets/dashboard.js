...
let simulationOn = window.devopsApp?.getSimulationState?.() ?? true;
...
function updateControls() {
  const role = getRole();
  if (triggerRunBtn) {
    triggerRunBtn.hidden = role !== "admin";
  }
  if (toggleSimBtn) {
    const isAdmin = role === "admin";
    const isOperator = role === "operator";
    const shouldShow = isAdmin || (isOperator && !simulationOn);
    toggleSimBtn.hidden = !shouldShow;
    if (!toggleSimBtn.hidden) {
      if (isOperator) {
...
if (toggleSimBtn) {
  toggleSimBtn.addEventListener("click", () => {
    ...
    } else if (role === "operator" && !simulationOn) {
      simulationOn = true;
    } else {
      return;
    }
...
document.addEventListener("role:change", updateControls);

document.addEventListener("simulation:change", (event) => {
  simulationOn = event.detail.isRunning;
  setRunState(simulationOn ? "Running" : "Paused", simulationOn ? "running" : "paused");
  updateControls();
});
